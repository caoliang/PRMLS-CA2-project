...
fit_df = pd.DataFrame(data=[['outlier images', unfit_img_df.shape[0]], 
                            ['remaining images', fit_img_df.shape[0]]],
...
ax = fit_img_df['class_name'].value_counts().transpose().plot(kind='bar', 
               rot=0, color=['#CC00CC', '#0000BB', 'green'])
for p in ax.patches:
    ax.annotate(str(p.get_height()), 
                (p.get_x() * 1.005, p.get_height() * 1.005))
plt.show()
